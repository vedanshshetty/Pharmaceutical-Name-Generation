...
class RiskBand(str, Enum):
    """Where a candidate sits relative to the two published POCA operating points.

    Reported explicitly because a `MODERATE` candidate previously passed silently: the
    headline `distinctiveness_margin` measures headroom to the *high* cutoff only, so a
    name scoring 57 against a 55 review line looked comfortable. Both margins are now
    carried, and the band names the operating point that actually applies.
    """
    LOW = "low"            # composite < moderate cutoff
    MODERATE = "moderate"  # moderate <= composite < high  (POCA: warrants human review)
    HIGH = "high"          # composite >= high cutoff      (POCA: unacceptable)
...
class FailureCode(str, Enum):
    ...
    STEM_FOREIGN_EMBEDDED = "STEM_FOREIGN_EMBEDDED"    # generic carries another class's stem internally
...
class SimilarityCheck(CheckOutcome):
    ...
    distinctiveness_margin_moderate: Optional[float] = Field(
        default=None,
        description="moderate_cutoff - nearest_match_score. Negative means the candidate "
                    "is inside the review band even though it cleared the hard cutoff.")
...
class QualityComponent(BaseModel):
    """One named term of the NameQuality objective, on a 0-1 scale before weighting."""
    model_config = ConfigDict(populate_by_name=True)
    name: str
...
class QualityReport(BaseModel):
    """How *good* a name is, as distinct from how *risky* it is.

    The verifier answers 'may this name exist'. This answers 'is this a name a human
    would actually want', which nothing in the v1.0 pipeline measured. Carried on the
    response so a shortlist can be ranked without re-deriving it.
    """
    model_config = ConfigDict(populate_by_name=True)
    total: float = Field(default=0.0, description="Weighted 0-100 composite, higher is better.")
    components: List[QualityComponent] = Field(default_factory=list)
    profile: str = Field(default="generic", description="'generic' | 'brand'")
    disqualified: bool = False
    notes: List[str] = Field(default_factory=list)

    def component(self, name: str) -> Optional[QualityComponent]:
...
class VerifierResponse(BaseModel):
    ...
    quality: Optional[QualityReport] = Field(
        default=None,
        description="Populated by the orchestrator, not the verifier: the verifier rules "
                    "on admissibility, the quality objective ranks what survives.")
